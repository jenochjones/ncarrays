...
class ARRAY:
    ...
    def get_series(self, path_to_geojson: str or None = None, axis: list or str = [], dimensional_values: dict = {}, statistic: str = 'mean', datetime_to_string: bool = True, datetime_format: str = '%m-%d-%Y %H:%M:%S'):
        subset_for_main_variable = []
        axis_dimension_type = None
        
        if path_to_geojson is not None:
            if len(self.y_dim) >= 0 or len(self.x_dim) >= 0 :
                x_y_array, extents = _geojson_to_raster(path_to_geojson, self.x_dim, self.y_dim, self.grid_mapping_variable)
        ...
        if (isinstance(axis, str) and axis is not None) or (isinstance(axis, list) and len(axis) == 1):
            ...
            series = _add_dimensional_axis(self.variables[axis_name]['dataset'], axis_dimension_values, series, axis_dimension_type, datetime_to_string, datetime_format)
    ...
    def get_spatial_info(self):
        ...
        return {'x_dim': {'max': max_x_dim, 'min': min_x_dim, 'interval': x_interval}, 'y_dim': {'max': max_y_dim, 'min': min_y_dim, 'interval': y_interval}}
